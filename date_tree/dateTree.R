...
# read in tree
tree<-read.tree("S2017S2023genbank_monophyly.treefile")
...
ml.tree.noCx <- drop.tip(ml.tree,"Culex_Culex_quinquefasciatus")
...
# find nodes and add ages (for tree without Culex)
nodes.2<-c(findMRCA(ml.tree.noCx,c("Psorophora_Grabhamia_cingulata","Psorophora_Psorophora_howardii")), #2
         findMRCA(ml.tree.noCx,c("Aedes_Stegomyia_aegypti","Aedes_Stegomyia_albopictus")), #3
         findMRCA(ml.tree.noCx,c("Verrallina_Verrallina_lineata","Aedes_Aedes_cinereus")), #4
         findMRCA(ml.tree.noCx,c("Aedes_Fredwardsius_vittatus","Opifex_Opifex_fuscus")), #5
         findMRCA(ml.tree.noCx,c("Aedes_Ochlerotatus_spencerii","Aedes_OchlerotatusChrysoconops_pallens")), #6
         findMRCA(ml.tree.noCx,c("Aedes_Fredwardsius_vittatus","Aedes_Aedimorphus_centropunctatus")), #7
         findMRCA(ml.tree.noCx,c("Aedes_OchlerotatusFinlayaSA_candidoscutellum","Aedes_Rampamyia_quinquelineatus")), #8
         findMRCA(ml.tree.noCx,c("Aedes_Kompia_purpureipes","Aedes_Lewnielsenius_muelleri")), #9
         findMRCA(ml.tree.noCx,c("Aedes_OchlerotatusCulicada_canadensis","Aedes_Ochlerotatus_fitchii")), #10
         findMRCA(ml.tree.noCx,c("Haemagogus_Conopostegus_leucocelaenus","Aedes_OchlerotatusCulicelsa_taeniorhynchus"))) #11
age.min.2=c(28.9922,25.0253,25.1549,52.0234,
          26.0171,37.4407,25.0169,14.6118,22.8726,26.4)
age.max.2=c(44.8503,38.4289,39.3621,77.4113,
          38.3484,55.3936,37.0298,27.603,33.9044,39.1343)
age.mean.2=c(35.8929,30.9185,31.5328,62.7468,
           31.4469,45.212,30.2981,20.7457,27.7271,31.9943)
...
# without cx pip outgroup
calibration.relative.2<-makeChronosCalib(ml.tree.noCx,node=nodes.2,age.min=age.min.2,age.max=age.max.2)
calibration.relative.2

calibration.absolute.2<-makeChronosCalib(ml.tree.noCx,node=nodes.2,age.min=age.mean.2,age.max=age.mean.2)
calibration.absolute.2
...
pl.tree.rel.2<-chronos(ml.tree.noCx,lambda=1,calibration=calibration.relative.2,model="relaxed",
                     control = chronos.control(iter.max=1e4,dual.iter.max=20))

pl.tree.abs.2<-chronos(ml.tree.noCx,lambda=1,calibration=calibration.absolute.2,model="relaxed",
                     control = chronos.control(iter.max=1e4,dual.iter.max=20))
...
# Read in time calibrated tree and subset RHK 2009 tips
setwd("/path/to/working/directory")
ml.tree<-read.tree("S2017S2023genbank_subgen.chronos.treefile")
sp.list<-read.csv("./constraint_tree/list_RHK.csv", header=T, row.names = 1) #read in list of RHK species
rhk.ml<-geiger::treedata(ml.tree, sp.list, sort=TRUE, warnings=TRUE)
tree<-rhk.ml$phy #153 tips 152 internal nodes
...
names(names.df)
str(names.df)
old<-names.df$standardizedNames
new<-names.df$names
